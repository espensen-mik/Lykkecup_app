  ...
  return (
    <div className="space-y-5">
      ...
      <section className="overflow-hidden rounded-lg border border-lc-border bg-white shadow-lc-card dark:border-gray-700 dark:bg-gray-900/35 dark:shadow-none">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-lc-border bg-gray-50/80 px-5 py-3.5 dark:border-gray-700 dark:bg-gray-800/40">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {data?.updated ? `Senest opdateret: ${formatDaDateTime(data.updated)}` : "Opdaterer..."}
          </p>
        </div>
          ...
          <ul className="divide-y divide-lc-border dark:divide-gray-700">
            {data.groups.map((g) => (
              <li key={g.key} className="flex items-center justify-between gap-3 px-5 py-3">
                ...
                <span className="text-sm font-semibold tabular-nums text-[#0f766e] dark:text-teal-300">{g.sold}</span>
              </li>
            ))}
          </ul>
        ) : (
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <article className="rounded-xl border border-teal-200/80 bg-white px-5 py-4 shadow-sm dark:border-teal-900/50 dark:bg-gray-900/35">
        ...
        <article className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm dark:border-gray-700 dark:bg-gray-900/35">
